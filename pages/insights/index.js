  ...
  return (
    <div style={{ background:'#fff', borderRadius:12, padding:'16px 18px', boxShadow:'0 1px 4px rgba(0,0,0,.08)' }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
        <h3 style={{ margin:0, fontSize:'.75rem', fontWeight:700, textTransform:'uppercase', letterSpacing:'.8px', color:'#888' }}>{title}</h3>
        {badge && <span style={{ fontSize:'.62rem', fontWeight:700, background:'#e8f5e9', color:'#4A7C59', padding:'2px 7px', borderRadius:20 }}>{badge}</span>}
      </div>
      {children}
    </div>
  );
...
function Stat({ label, value, sub, color }) {
  return (
    <div style={{ textAlign:'center', flex:'1 1 80px', minWidth:0 }}>
      <div style={{ fontSize:'1.6rem', fontWeight:800, color:color||'#1a1a1a', lineHeight:1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{value}</div>
  ...
  return (
    <div style={{ marginBottom:9 }}>
      <div style={{ display:'flex', justifyContent:'space-between', fontSize:'.75rem', marginBottom:3 }}>
        <span style={{ color:'#333', fontWeight:500, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'60%' }}>{label}</span>
        <span style={{ color:'#888', fontWeight:600, flexShrink:0 }}>{fmt(count)}{sub ? ` · ${sub}` : ''}</span>
      </div>
...
function WaInbox() {
  ...
  const openConv = (phone, unread) => {
    setSelected(phone);
    if (unread) markRead(phone);
  ...
  if (error) return (
    <div style={{ background:'#fff8e1', border:'1px solid #ffe082', borderRadius:10, padding:'16px 18px' }}>
  ...
  const Sidebar = (
    <div style={{
      ...
      <div style={{ padding:'12px 16px', borderBottom:'1px solid #f0f0f0', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
        ...
        <button onClick={load} style={{ background:'none', border:'none', cursor:'pointer', color:'#4A7C59', fontWeight:700, fontSize:'.9rem', padding:'4px 8px' }}>↻</button>
      </div>
      <div style={{ overflowY:'auto', flex:1 }}>
        {conversations.map(c => {
          ...
          return (
            <div key={c.phone} onClick={() => openConv(c.phone, c.unread)} style={{
              padding:'12px 16px', cursor:'pointer', borderBottom:'1px solid #f0f0f0',
              background: selected === c.phone && !isMobile ? '#e8f5e9' : 'transparent',
              transition:'background .15s',
            }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <span style={{ fontWeight: c.unread ? 800 : 600, fontSize:'.85rem', color:'#1a1a1a', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'70%' }}>
                  {c.name}
                </span>
                <span style={{ fontSize:'.68rem', color:'#aaa', flexShrink:0 }}>{fmtTime(c.lastAt)}</span>
              </div>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:3 }}>
                <span style={{ fontSize:'.75rem', color:'#888', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'80%' }}>{preview}</span>
  ...
  const ChatPane = (
    <div style={{
      ...
      {activeConv ? (
        <>
          {/* Header */}
          <div style={{ padding:'10px 16px', borderBottom:'1px solid #f0f0f0', background:'#fff', display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
            {isMobile && (
              <button onClick={() => setShowChat(false)} style={{ background:'none', border:'none', fontSize:'1.2rem', cursor:'pointer', padding:'0 4px', color:'#4A7C59', fontWeight:700 }}>
                ←
              </button>
            )}
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontWeight:800, fontSize:'.88rem', color:'#1a1a1a', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{activeConv.name}</div>
              <div style={{ fontSize:'.7rem', color:'#888' }}>{activeConv.phone}</div>
            </div>
            <a href={`https://wa.me/${activeConv.phone}`} target="_blank" rel="noreferrer"
              style={{ fontSize:'.72rem', background:'#25d366', color:'#fff', padding:'5px 10px', borderRadius:8, fontWeight:700, textDecoration:'none', flexShrink:0, whiteSpace:'nowrap' }}>
              {isMobile ? 'WA ↗' : 'Open in WA ↗'}
            </a>
          </div>

          {/* Messages */}
          <div ref={chatRef} style={{ flex:1, overflowY:'auto', padding:'12px 14px', background:'#ece5dd', display:'flex', flexDirection:'column', gap:8 }}>
                ...
                <React.Fragment key={msg.id || i}>
                  {!isOut && (
                    <div style={{ display:'flex', justifyContent:'flex-start' }}>
                      <div style={{ maxWidth:'82%', background:'#fff', borderRadius:'12px 12px 12px 2px', padding:'8px 11px', boxShadow:'0 1px 2px rgba(0,0,0,.08)' }}>
                        <div style={{ fontSize:'.84rem', color:'#1a1a1a', lineHeight:1.5, whiteSpace:'pre-wrap' }}>{msg.message}</div>
                        <div style={{ fontSize:'.62rem', color:'#aaa', marginTop:3, textAlign:'right' }}>{fmtTime(msg.created_at)}</div>
                      </div>
                    </div>
                  )}
                  {isBot && (
                    <div style={{ display:'flex', justifyContent:'flex-end' }}>
                      <div style={{ maxWidth:'82%', background:'#d9f7be', borderRadius:'12px 12px 2px 12px', padding:'8px 11px', boxShadow:'0 1px 2px rgba(0,0,0,.08)' }}>
                        <div style={{ fontSize:'.62rem', color:'#888', marginBottom:3, fontWeight:600 }}>🤖 Bot</div>
                        <div style={{ fontSize:'.84rem', color:'#1a1a1a', lineHeight:1.5, whiteSpace:'pre-wrap' }}>{msg.bot_replied}</div>
                        <div style={{ fontSize:'.62rem', color:'#888', marginTop:3, textAlign:'right' }}>✓✓</div>
                      </div>
                    </div>
                  )}
                  {isOut && (
                    <div style={{ display:'flex', justifyContent:'flex-end' }}>
                      <div style={{ maxWidth:'82%', background:'#dcf8c6', borderRadius:'12px 12px 2px 12px', padding:'8px 11px', boxShadow:'0 1px 2px rgba(0,0,0,.08)' }}>
                        <div style={{ fontSize:'.62rem', color:'#4A7C59', marginBottom:3, fontWeight:700 }}>You</div>
                        <div style={{ fontSize:'.84rem', color:'#1a1a1a', lineHeight:1.5, whiteSpace:'pre-wrap' }}>{msg.message}</div>
                        <div style={{ fontSize:'.62rem', color:'#888', marginTop:3, textAlign:'right' }}>{fmtTime(msg.created_at)} ✓✓</div>
                      </div>
                    </div>
                  )}
                </React.Fragment>
          ...
          {/* Reply input */}
          <form onSubmit={sendReply} style={{ padding:'10px 12px', borderTop:'1px solid #f0f0f0', background:'#f8f8f8', display:'flex', gap:8, alignItems:'flex-end', flexShrink:0 }}>
            <div style={{ flex:1 }}>
              <textarea
                value={reply}
...
export default function Insights() {
  ...
  return (
    <>
      ...
      <div style={{ minHeight:'100vh', background:'#f5f5f5', fontFamily:'system-ui,sans-serif', padding: isMobile ? '16px 12px' : '28px 16px' }}>
        <div style={{ maxWidth:960, margin:'0 auto' }}>
          ...
          <div style={{ marginBottom:16 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
              <div>
                <h1 style={{ margin:0, fontSize: isMobile ? '1.2rem' : '1.5rem', fontWeight:800, color:'#1a1a1a' }}>📊 Vedayu Insights</h1>
                <p style={{ margin:'3px 0 0', color:'#888', fontSize:'.75rem' }}>
                  {lastUpdated ? `Updated ${lastUpdated.toLocaleTimeString('en-IN')}` : 'Loading...'}
                </p>
              </div>
              <div style={{ display:'flex', gap:6 }}>
                <button onClick={load} disabled={loading} style={{ background:'#4A7C59', color:'#fff', border:'none', borderRadius:8, padding:'7px 12px', fontSize:'.8rem', fontWeight:700, cursor:loading?'not-allowed':'pointer', opacity:loading?0.6:1 }}>
                  {loading ? '⟳' : '⟳'}
                </button>
                <a href="/api/insights-auth?logout=1" style={{ padding:'7px 12px', borderRadius:8, fontWeight:700, fontSize:'.8rem', background:'#f0f0f0', color:'#888', textDecoration:'none', display:'flex', alignItems:'center' }}>
                  {isMobile ? '⎋' : 'Logout'}
                </a>
              </div>
            </div>
            ...
            <div style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:2, WebkitOverflowScrolling:'touch' }}>
              <button style={btnStyle(tab==='ga')}      onClick={() => setTab('ga')}>
                {isMobile ? '📈 GA' : 'Google Analytics'}
              </button>
              <button style={btnStyle(tab==='clarity')} onClick={() => setTab('clarity')}>
                {isMobile ? '🖱 Clarity' : 'Clarity'}
              </button>
              <button style={btnStyle(tab==='wa')}      onClick={() => setTab('wa')}>
                💬 WhatsApp
              </button>
            </div>
          </div>
          ...
          {error && tab !== 'wa' && (
            <div style={{ background:'#fee', border:'1px solid #fcc', borderRadius:8, padding:'10px 14px', color:'#c00', marginBottom:14, fontSize:'.82rem' }}>Error: {error}</div>
          )}
          {loading && !ga && !clarity && tab !== 'wa' && (
            <div style={{ textAlign:'center', padding:60, color:'#888' }}>Loading insights...</div>
          )}
