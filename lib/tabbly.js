...
/**
 * Returns the current IST hour (0-23) using UTC math.
 * IST = UTC+5:30. Avoids Intl.DateTimeFormat locale quirks on Linux/Node.
 */
function istHourNow() {
  const nowMs  = Date.now();
  const istMs  = nowMs + 5.5 * 60 * 60 * 1000; // +5:30
...
export function nextCallWindowStart() {
  ...
  // Build today's 10:15 AM IST as UTC ms
  const todayAt1015Utc =
    Date.UTC(istNow.getUTCFullYear(), istNow.getUTCMonth(), istNow.getUTCDate(), 4, 45); // 10:15 IST = 04:45 UTC

  if (todayAt1015Utc > now.getTime()) return new Date(todayAt1015Utc);
  return new Date(todayAt1015Utc + 24 * 60 * 60 * 1000); // tomorrow
}
