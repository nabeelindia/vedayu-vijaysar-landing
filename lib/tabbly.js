...
const TABBLY_CAMPAIGN_ID = process.env.TABBLY_CAMPAIGN_ID;
const TABBLY_AGENT_ID   = process.env.TABBLY_AGENT_ID;
...
export async function triggerTabblyCall({ orderId, name, price, address, state: customerState, mobile }) {
  if (!TABBLY_API_KEY || !TABBLY_CAMPAIGN_ID || !TABBLY_AGENT_ID) {
  ...
  const res = await fetch(TABBLY_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      campaign_id:          TABBLY_CAMPAIGN_ID,
