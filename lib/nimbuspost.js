...
const B2B_BASE_URL = 'https://ship.nimbuspost.com/api';
...
/**
 * Track a single B2B shipment using the ship.nimbuspost.com API.
 * Returns the full data object: { status, rto_status, edd, history[], ... }
 * This is the authoritative tracking endpoint per the NimbusPost B2B API docs.
 */
export async function trackShipment(awb) {
  const token = await getToken();

  const doFetch = async (tok) =>
    fetch(`${B2B_BASE_URL}/shipmentcargo/track/${encodeURIComponent(awb)}`, {
  ...
  // 401 → clear cached token, retry once with fresh credentials
  if (res.status === 401) {
    await kv.del(TOKEN_KV_KEY).catch(() => {});
    const fresh = await fetchToken();
    await kv.set(TOKEN_KV_KEY, fresh, { ex: 82800 }).catch(() => {});
    res  = await doFetch(fresh);
    json = await res.json();
  }

  if (!json.status) {
    throw new Error(`NimbusPost trackShipment failed for ${awb}: ${json.message || JSON.stringify(json)}`);
  }
  return json.data;
}
