...
// ─── KV Mappings for Tracking ────────────────────────────────────────────────

/**
 * Store orderId → awb and phone/email → [orderIds] in KV.
 * This lets the tracking page look up by Order ID, phone, or email.
 */
export async function storeAwbMapping({ orderId, awb, shipmentId, mobile, email, courierName }) {
  ...
  try {
    // orderId → awb (180 day TTL)
    await kv.set(`nimbuspost:order:${orderId}`, record, { ex: 15552000 });

    // phone → list of orderIds
    if (mobile) {
      await kv.lpush(`nimbuspost:phone:${mobile}`, orderId);
      await kv.expire(`nimbuspost:phone:${mobile}`, 15552000);
    }

    // email → list of orderIds
    if (email) {
      const emailKey = email.toLowerCase().trim();
      await kv.lpush(`nimbuspost:email:${emailKey}`, orderId);
      await kv.expire(`nimbuspost:email:${emailKey}`, 15552000);
    }
  } catch (err) {
...
export async function getOrdersByPhone(mobile) {
  try {
    const orderIds = await kv.lrange(`nimbuspost:phone:${mobile}`, 0, 9);
    return orderIds || [];
  } catch (_) { return []; }
}
...
export async function getOrdersByEmail(email) {
  try {
    const orderIds = await kv.lrange(`nimbuspost:email:${email.toLowerCase().trim()}`, 0, 9);
    return orderIds || [];
  } catch (_) { return []; }
}
