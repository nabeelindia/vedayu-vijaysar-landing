...
create table if not exists growth_partners (
  ...
  profession    text        not null check (profession in ('Doctor','Nutritionist','Yoga Instructor','Influencer','Other')),
  city          text        not null,
...
create table if not exists gp_earnings (
  ...
  status       text        not null default 'pending' check (status in ('pending','in_transit','earned','cancelled')),
